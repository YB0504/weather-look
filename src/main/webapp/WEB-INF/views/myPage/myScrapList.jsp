...
<main id="main" class="main">
	...
	<div class="pagetitle" style="margin-bottom: 30px">
		...
		<nav>
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="index.html">스크랩</a></li>
			</ol>
		</nav>
	</div><!-- End Page Title -->
			...
			<c:if test="${not empty scrapList}">
				...
				<c:forEach var="scrap" items="${scrapList}">
					<tr>
						...
						<td><a href="detail?type_name=${scrap.board_type}&post_id=${scrap.post_id}">${scrap.title}</a></td>
						<td>${scrap.nick}</td>
						<td>
					...
					<ul class="pagination">
						...
						<c:if test="${startPage > 10}">
							<li class="page-item disabled">
								<a class="page-link" href="myScrapList?page=${startPage - 10}" tabindex="-1" aria-disabled="true">&laquo;</a>
							</li>
						</c:if>
						<!-- 각 블럭에 10개의 페이지 출력 -->
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i != page }"> <!-- 현재 페이지가 아닌 경우 -->
								<li class="page-item"><a class="page-link" href="myScrapList?page=${i}">${i}</a></li>
							</c:if>
						</c:forEach>
						...
						<c:if test="${endPage < totalPage}">
							<li class="page-item">
								<a class="page-link" href="myScrapList?page=${endPage+1}">&raquo;</a>
							</li>
						</c:if>
					</ul>
				</div>
